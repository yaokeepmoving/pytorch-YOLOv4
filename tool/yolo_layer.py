...
def yolo_forward(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
    ...
    # Shape: [batch, num_anchors, num_classes, H * W] --> [batch, num_anchors
    # * H * W, num_classes]
    cls_confs = cls_confs.permute(
        0, 1, 3, 2).reshape(
    ...
    # Shape: [batch, num_anchors * h * w, 4] -> [batch, num_anchors * h * w,
    # 1, 4]
    boxes = torch.cat(
        (bx1, by1, bx2, by2), dim=2).view(
...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        ...
        return yolo_forward_dynamic(output, self.thresh, self.num_classes, masked_anchors, len(
            self.anchor_mask), scale_x_y=self.scale_x_y)
