...
def bboxes_iou_test(bboxes_a, bboxes_b, fmt='voc', iou_type='iou'):
    ...
    if fmt.lower() == 'voc':  # xmin, ymin, xmax, ymax
        ...
        tl_intersect = torch.max(
            bboxes_a[:, np.newaxis, :2], bboxes_b[:, :2])  # of shape `(N,K,2)`
        # bottom right
    ...
    if N == K == 1:
        ...
        product_of_lengths = torch.norm(bb_a, p='fro', dim=1)[
            :, np.newaxis] * torch.norm(bb_b, p='fro', dim=1)
        ...
        bc.xmin, bc.ymin, bc.xmax, bc.ymax = bc.xmin - adjust_x, bc.ymin - \
            adjust_y, bc.xmax - adjust_x, bc.ymax - adjust_y

        ba, bb = bboxes_a.numpy().astype(
            int)[0], bboxes_b.numpy().astype(int)[0]
        if fmt.lower() == 'voc':  # xmin, ymin, xmax, ymax
            ba = ED({"xmin": ba[0] - adjust_x,
                     "ymin": ba[1] - adjust_y,
                     "xmax": ba[2] - adjust_x,
                     "ymax": ba[3] - adjust_y})
            bb = ED({"xmin": bb[0] - adjust_x,
                     "ymin": bb[1] - adjust_y,
                     "xmax": bb[2] - adjust_x,
                     "ymax": bb[3] - adjust_y})
        elif fmt.lower() == 'yolo':  # xcen, ycen, w, h
            ba = ED({"xmin": ba[0] - ba[2] // 2 - adjust_x,
                     "ymin": ba[1] - ba[3] // 2 - adjust_y,
                     "xmax": ba[0] + ba[2] // 2 - adjust_x,
                     "ymax": ba[1] + ba[3] // 2 - adjust_y})
            bb = ED({"xmin": bb[0] - bb[2] // 2 - adjust_x,
        ...
        plane = np.full(shape=(int(1.5 * (bc.ymax - bc.ymin)),
                               int(1.5 * (bc.xmax - bc.xmin)),
                               3),
                        fill_value=255,
                        dtype=np.uint8)
        ...
        line_size = 1
        if cv2:
            cv2.rectangle(img_with_boxes, (ba.xmin, ba.ymin),
                          (ba.xmax, ba.ymax), (0, 255, 0), line_size)
            cv2.rectangle(img_with_boxes, (bb.xmin, bb.ymin),
                          (bb.xmax, bb.ymax), (0, 0, 255), line_size)
            cv2.rectangle(img_with_boxes, (max(0, bc.ymin -
                                               1), max(0, bc.ymin -
                                                       1)), (bc.xmax, bc.ymax), (255, 0, 0), line_size)
        else:
            ...
            # drawer.line([((max(0,bc.xmin-1), max(0,bc.ymin-1)),
            # ((max(0,bc.xmin-1), bc.ymax), (bc.xmax, bc.ymax), (bc.xmax,
            # max(0,bc.ymin-1)), ((max(0,bc.xmin-1), max(0,bc.ymin-1))],
            # fill='red', width=line_size)
            drawer.rectangle(
                [(ba.xmin, ba.ymin), (ba.xmax, ba.ymax)], outline='green', width=line_size)
            drawer.rectangle(
                [(bb.xmin, bb.ymin), (bb.xmax, bb.ymax)], outline='blue', width=line_size)
            drawer.rectangle([(max(0, bc.xmin - 1), max(0, bc.ymin - 1)),
                              (bc.xmax + 1, bc.ymax + 1)], outline='red', width=line_size)
